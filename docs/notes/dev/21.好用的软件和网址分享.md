...
4. Macos上的改建神器 [Karabiner-Elements](https://karabiner-elements.pqrs.org/)，当你的键盘不支持 Macos 系统时，或者可以使用这个软件调换 cmd 和 alt，还有其他的神器功能<Badge text="MacOS" type="tip" />；
...
28. 自动切换输入法，终于可以不用经常切换输入法了[Input Source Pro](https://inputsource.pro/zh-CN)<Badge text="MacOS" type="tip" />；
29. 开源免费高颜值的 Homebrew UI 工具[Applite](https://github.com/milanvarady/Applite)<Badge text="MacOS" type="tip" />；
30. 强烈推荐微信输入法，让你的隐私只给到[微信输入法](https://z.weixin.qq.com/)<Badge text="MacOS" type="tip" /><Badge text="Windows" type="info" />；


## 命令行工具推荐

1. `ls`命令的完美的替代品 [eza](https://github.com/eza-community/eza)；
2. 在命令行压测 http 请求的简单高效工具 [oha](https://github.com/hatoo/oha)；
3. 由AI大型语言模型（LLM）提供支持的命令行生产力工具 [ShellGPT](https://github.com/TheR1D/shell_gpt)；
4. [http-server](https://github.com/http-party/http-server)是一个简单的、零配置的命令行静态HTTP服务器。它对于生产使用来说足够强大，但它足够简单和可破解，可以用于测试，本地开发和学习；
5. 一款资源监视器命令行工具，可显示处理器、内存、磁盘、网络和进程的使用情况和统计信息 [btop](https://github.com/aristocratos/btop)；
6. [K9s](https://github.com/derailed/k9s) 提供了一个终端用户界面，用于与你的 Kubernetes 集群进行交互。这个项目的目的是让在实际环境中导航、观察和管理你的应用程序变得更容易。K9s 持续观察 Kubernetes 的变化，并提供后续命令来与你观察到的资源进行交互。
7. [ripgrep](https://github.com/BurntSushi/ripgrep) 是 grep 的增强替代品；
